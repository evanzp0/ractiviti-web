	 ...
	 if (container != null)
	 {
		 ...
		 this.moveHandler = mxUtils.bind(this, function(evt)
		 {
			 ...
			 if (this.captureDocumentGesture && graph.isMouseDown && graph.container != null &&
				 ...
				 var fireMouseEvent= graph.fireMouseEvent,
					 evtName = mxEvent.MOUSE_MOVE,
					 x = null;
				 if (mxClient.IS_TOUCH) {
					 x = mxEvent.getClientX(evt);
					 var y = mxEvent.getClientY(evt);
					 var point = mxUtils.convertPoint(container, x, y);
					 var state = graph.view.getState(graph.getCellAt(point.x, point.y));
				 }
				 fireMouseEvent.call(graph, evtName, new mxMouseEvent(evt, state));
			 }
		 });
