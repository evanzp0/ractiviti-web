...
{
	var sb = this;
	var req = mxUtils.load(window.STENCIL_PATH + "/templates.xml");
	var fns = this.createTemplateEntryFromXmlArray(req.getDocumentElement().children);
...
Sidebar.prototype.dropAndConnect = function(source, targets, direction, dropCellIndex, evt)
{
	...
	if (geo != null)
	{
		...
		try
		{
			...
			{
				var templates = this.graph.templates;
				var defaultEdgeKey = this.graph.defaultEdgeKey;
				var defaultEdgeValue = null;
				if (templates != null) {
					var defaultEdge = templates[defaultEdgeKey];
					defaultEdgeValue = defaultEdge.value;
				}
				...
				targets.push(graph.insertEdge(null, null, defaultEdgeValue, source, targets[dropCellIndex],
...
Sidebar.prototype.createTemplateEntryFromXmlArray = function(xml_arr) {
	var entries = [];
	...
	for (var i = 0; i < xml_arr.length; i++) {
		var template_el = xml_arr[i];
		var name = template_el.getAttribute("name");
		var title = template_el.getAttribute("title");
		var tag = template_el.getAttribute("tag");
		
		var codec = new mxCodec();
		var cell = codec.decodeCell(template_el.firstElementChild);

		editor.templates[name] = cell;

		if (cell.isVertex()) {
			var entry = this.createVertexTemplateEntry(cell.style, cell.geometry.width, cell.geometry.height, cell.value, title);
			entries.push(entry);
		} else if (cell.isEdge()) {
			var entry = this.createEdgeTemplateEntry(cell.style, cell.geometry.width, cell.geometry.height, cell.value, title);
			entries.push(entry);
		}
	}

	editor.graph["templates"] = editor.templates;
